...
// 静态注册表
static std::vector<Component75*> g_registry;
...
std::string Component75::getName() const {
    return name_;
}

int Component75::getVersion() const {
    return version_;
}

void Component75::setDependency(std::shared_ptr<void> dep) {
    if (dep) {
        dependencies_.push_back(dep);
    }
}

void Component75::validate() {
    if (name_.empty()) {
...
std::shared_ptr<Component75> createComponent75() {
    auto component = std::make_shared<Component75>();
