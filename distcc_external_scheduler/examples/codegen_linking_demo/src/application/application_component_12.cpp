#include "application_component_12.h"
#include <iostream>
#include <stdexcept>
#include <algorithm>
#include <sstream>
...
// 静态注册表
static std::vector<Component12*> g_registry;
...
void Component12::initialize() {
    ...
    // 使用依赖: middleware_component_13.h
...
std::string Component12::getName() const {
    return name_;
}

int Component12::getVersion() const {
    return version_;
}

void Component12::setDependency(std::shared_ptr<void> dep) {
    if (dep) {
        dependencies_.push_back(dep);
    }
}

void Component12::validate() {
    if (name_.empty()) {
...
std::shared_ptr<Component12> createComponent12() {
    auto component = std::make_shared<Component12>();
