...
// 静态注册表
static std::vector<Component0*> g_registry;
...
std::string Component0::getName() const {
    return name_;
}

int Component0::getVersion() const {
    return version_;
}

void Component0::setDependency(std::shared_ptr<void> dep) {
    if (dep) {
        dependencies_.push_back(dep);
    }
}

void Component0::validate() {
    if (name_.empty()) {
...
std::shared_ptr<Component0> createComponent0() {
    auto component = std::make_shared<Component0>();
